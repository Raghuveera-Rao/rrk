...
abstract class AbstractSystemTest {
    ...
    static void startpravegaInstances(final URI zkUri) {
        Service prService = Utils.createPravegaService(zkUri);
        if (!prService.isRunning()) {
            prService.start(true);
        }
        List<URI> prUris = prService.getServiceDetails();
        log.debug("Pravega service details: {}", prUris);
    }
