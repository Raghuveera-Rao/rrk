 ...
 */
@Slf4j
abstract class AbstractComponentScaleTest extends AbstractReadWriteTest {
    ...
    Service bookkeeperInstance;
    Service zookeeperInstance;
    URI controllerURIDirect = null;
    Controller controller;
    ScheduledExecutorService controllerExecutorService;

    void scaleComponentTest(Map<String, Integer> scaleMap) throws ExecutionException {
        log.info("Test before scaling scenario");
        long currentWriteCount1 = testState.getEventWrittenCount();
        long currentReadCount1 = testState.getEventReadCount();
        log.info("Read count: {}, write count: {} without any scaling", currentReadCount1, currentWriteCount1);
        ...
        log.info("Read count: {}, write count: {} after sleep before scaling", currentReadCount2, currentWriteCount2);
        ...
        if(scaleMap.containsKey("bookkeeper")){
            int scale = scaleMap.get("bookkeeper");
            Futures.getAndHandleExceptions(bookkeeperInstance.scaleService(scale), ExecutionException::new);
            //zookeeper will take about 30 seconds to detect that the node has gone up/down
            Exceptions.handleInterrupted(() -> Thread.sleep(WAIT_AFTER_FAILOVER_MILLIS));
            log.info("Scaling bookkeeper instances to {}",scale);
        }

        if(scaleMap.containsKey("controller")){
            int scale = scaleMap.get("controller");
            Futures.getAndHandleExceptions(controllerInstance.scaleService(scale), ExecutionException::new);
            //zookeeper will take about 30 seconds to detect that the node has gone up/down
            Exceptions.handleInterrupted(() -> Thread.sleep(WAIT_AFTER_FAILOVER_MILLIS));
            log.info("Scaling controller instances to {}",scale);
        }

        if(scaleMap.containsKey("segmentstore")){
            int scale = scaleMap.get("segmentstore");
            Futures.getAndHandleExceptions(segmentStoreInstance.scaleService(scale), ExecutionException::new);
            //zookeeper will take about 30 seconds to detect that the node has gone up/down
            Exceptions.handleInterrupted(() -> Thread.sleep(WAIT_AFTER_FAILOVER_MILLIS));
            log.info("Scaling segmentstore instances to {}",scale);
        }

        if(scaleMap.containsKey("zookeeper")){
            int scale = scaleMap.get("zookeeper");
            Futures.getAndHandleExceptions(zookeeperInstance.scaleService(scale), ExecutionException::new);
            //zookeeper will take about 30 seconds to detect that the node has gone up/down
            Exceptions.handleInterrupted(() -> Thread.sleep(WAIT_AFTER_FAILOVER_MILLIS));
            log.info("Scaling zookeeper instances to {}",scale);
        }

        currentWriteCount1 = testState.getEventWrittenCount();
        currentReadCount1 = testState.getEventReadCount();
        log.info("Read count: {}, write count: {} after scaling scenario after sleep", currentReadCount1, currentWriteCount1);
        //ensure writes are happening
        ...
        log.info("Read count: {}, write count: {} with scaling scenario after sleep", currentReadCount1, currentWriteCount1);
    }
